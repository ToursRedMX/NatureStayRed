...
--
--   Pattern for future bridge tables:
--   When trip_nature_stay_bookings is created (Block D), add an analogous
--   block checking for naturestayred/accommodation.
--   When trip_routesred_bookings is created, add a block for
--   routesred/transport.
-- ============================================================================
...
-- SELECT: links of own trips; super_admin/global sees all
...
--
-- Pattern for future bridge tables:
--   When trip_nature_stay_bookings is created, add:
--     IF EXISTS (SELECT 1 FROM ecosystem.trip_nature_stay_bookings nb
--                WHERE nb.trip_item_id = NEW.id) THEN
--       IF NEW.platform <> 'naturestayred' OR NEW.item_type <> 'accommodation' THEN
--         RAISE EXCEPTION '...';
--       END IF;
--     END IF;
--
--   When trip_routesred_bookings is created, add:
--     IF EXISTS (SELECT 1 FROM ecosystem.trip_routesred_bookings rb
--                WHERE rb.trip_item_id = NEW.id) THEN
--       IF NEW.platform <> 'routesred' OR NEW.item_type <> 'transport' THEN
--         RAISE EXCEPTION '...';
--       END IF;
--     END IF;
-- ============================================================================
